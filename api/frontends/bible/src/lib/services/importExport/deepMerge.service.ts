...
export class DeepMergeService {
	 ...
	 */
	filterInPlace<T>(array: Array<T>, predicate: (el: T) => boolean): Array<T> {
		let outputIndex = 0;
	...
	>(
		...
		return this.deepMergeInternal(record, other, new Set(), options);
	}
	...
	>(
		...
		for (const key of keys) {
			...
			if (
				this.isNonNullObject(a) &&
				this.isNonNullObject(b) &&
				!seen.has(a) &&
				!seen.has(b)
			) {
				seen.add(a);
				seen.add(b);
				result[key] = this.mergeObjects(a, b, seen, options) as ResultMember;
	...
	mergeObjects(
		...
		if (this.isMergeable(left) && this.isMergeable(right)) {
			return this.deepMergeInternal(left, right, seen, options);
		}

		if (this.isIterable(left) && this.isIterable(right)) {
			// Handle arrays
	...
	isIterable(value: NonNullable<unknown>): value is Iterable<unknown> {
	...
	getKeys<T extends Record<string, unknown>>(record: T): Array<keyof T> {
		const result = Object.getOwnPropertySymbols(record) as Array<keyof T>;
		this.filterInPlace(result, (key) =>
...
export const deepMergeService = new DeepMergeService();
